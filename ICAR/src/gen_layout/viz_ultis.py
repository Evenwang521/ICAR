...
import sys
sys.path.append("D:\code\python\POSA\src\gen_layout")
...
def viz_opti_eles_obbs(curr_obbs,ele_list,ele_pos,ele_orient,scene_mesh,body_mesh):
    ...
    for i in range(len(ele_list)):
        ...
        mesh = o3d.io.read_triangle_mesh(ele_list[i])
        # init_orient = torch.tensor([1.0, .0, .0],dtype=torch.float32) #
        # 元素初始朝向为(1,0,0)
        # axis = torch.cross(init_orient, ele_orient[i].clone().detach())
        # axis_norm = axis/torch.norm(axis)
        # angle = torch.acos(torch.dot(init_orient, ele_orient[i].clone().detach())/torch.norm(ele_orient[i])) #旋转角
...
if __name__ == "__main__":
    # ele_path_1 = r"D:\data\virtual_elements\3D_model\UI.obj"
    # ele_path_2 = r"D:\data\virtual_elements\rabbit\UI.obj"
    # ele_path = r"D:\data\virtual_elements\clock\UI.obj"
    # ele_path = r"D:\data\virtual_elements\menu\UI.obj"
    # mesh1 = o3d.io.read_triangle_mesh(ele_path_1)
    # mesh2 = o3d.io.read_triangle_mesh(ele_path_2)
    # mesh_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1)
    # o3d.visualization.draw_geometries([mesh1,mesh2,mesh_frame])
    # o3d.visualization.draw_geometries([mesh])
    pdb.set_trace()
